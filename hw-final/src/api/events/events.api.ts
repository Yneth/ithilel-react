import {CreateEventRequest, EventListResponse, EventResponse, UpdateEventRequest} from "./events.types";
import client from "../client";

const API_PREFIX = '/events';

class Events {
    public create = async (request: CreateEventRequest): Promise<EventResponse> => {
    ...
    public update = async (request: UpdateEventRequest): Promise<EventResponse> => {
        const {data} = await client.put<EventResponse>(`${API_PREFIX}/${request.id}`, request);
        return data;
    }
