...
const MakeBetForm: FC<{
  ...
  const selectedOutcome =
    (event?.outcomes?.length || 0) > selectedOutcomeIndex
      ? event.outcomes[selectedOutcomeIndex]
      : null;

  const onOutcomeSelected = useCallback(
    ...
    <Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
      ...
      <CardContent>
        <Typography align="center">
          Odds: {round(selectedOutcome?.odds, 2)}
        </Typography>
        ...
        <TextField
          ...
          helperText={
            betAmount <= 0
              ...
              : `Estimated earnings: +${calcEarnings(
                  betAmount,
      ...
      <CardActions sx={{ justifyContent: "center" }}>
        <Button color="success" onClick={onMakeBet}>
          Confirm
        </Button>
      </CardActions>
    </Paper>
