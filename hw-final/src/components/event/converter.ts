...
function createMockOutcome(name: string, type: BooleanOutcomeType): OutcomeResponse {
...
export function mapToCreateRequest(fields: EventFields): CreateEventRequest {
    return {
        ...
        backgroundUrl: fields.backgroundUrl ? fields.backgroundUrl : `https://cataas.com/c?q=${fields.name}`,
        outcomes: [createMockOutcome(fields.yesOutcomeName, 'yes'), createMockOutcome(fields.noOutcomeName, 'no')],
    };
}

export function mapToUpdateRequest(eventId: string, fields: EventFields): UpdateEventRequest {
    return {
