...
const UpsertEventForm: FC = memo(() => {
  ...
  const onSave = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const action = isCreate
        ...
        : eventActions.updateEventRequest(
            mapToUpdateRequest(eventId, validFields),
          );
      dispatch(action);
    },
  ...
  return (
    <Paper
      sx={{ p: 3, px: 6, display: "flex", flexDirection: "column", gap: 2 }}
      component="form"
      onSubmit={onSave}
    >
      <Typography variant="h5" align="center">
        {isCreate ? "Create new" : "Update"} Bet Event
      </Typography>
      <TextField
        variant="standard"
      ...
      <CardImage
        ...
        src={
          event.backgroundUrl
            ? event.backgroundUrl
            : `https://cataas.com/c?q=${event.name}`
        }
      />
      <TextField
        variant="standard"
      ...
      <TextField
        ...
        onChange={validateField(validateOutcomeName, setEvent, setFieldErrors)}
      />
      <Button type="submit" disabled={!areAllFieldsValid(fieldErrors)}>
