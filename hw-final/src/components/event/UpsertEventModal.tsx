...
import {eventActions, selectModalOptions} from "../../store/events/events.slice";

const UpsertEventModal: FC = memo(() => {
    const modalOptions = useAppSelector(selectModalOptions);
    const dispatch = useAppDispatch();

    return <Modal open={modalOptions.open} onClose={() => dispatch(eventActions.hideUpsertModal())}>
        <AbsoluteCenter>
            <UpsertEventForm/>
        </AbsoluteCenter>
    </Modal>
