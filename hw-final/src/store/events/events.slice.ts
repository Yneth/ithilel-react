...
const slice = createSlice({
  ...
  reducers: {
    ...
    openUpsertModal: (
      state,
      { payload }: PayloadAction<BetEventId | undefined>,
    ) => {
    ...
    fetchEventsSuccess: (
      state,
      { payload }: PayloadAction<EventListResponse>,
    ) => {
    ...
    createEventSuccess: (
      state,
      { payload }: PayloadAction<CreateEventRequest>,
    ) => {
      state.byId[payload.id] = payload;
    },
    ...
    updateEventSuccess: (
      state,
      { payload }: PayloadAction<UpdateEventRequest>,
    ) => {
      state.byId[payload.id] = payload;
    },
...
export const selectEvents = (state: RootState) =>
  Object.values(state.events.byId);
export const selectEventsFiltered = createSelector(
  [selectEvents, (state: RootState, query: string | null | undefined) => query],
  (events: EventListResponse, query: string | null | undefined) =>
    query
      ? events.filter(
          (event) => event.name.toLowerCase().indexOf(query.toLowerCase()) >= 0,
        )
      : events,
);
export const selectEvent = (eventId: BetEventId) => (state: RootState) =>
  state.events.byId[eventId];
export const selectMaybeEvent =
  (eventId?: BetEventId) =>
  (state: RootState): BetEvent | undefined => {
    if (eventId) {
      return state.events.byId[eventId];
    }
  };
export const selectModalOptions = (state: RootState) =>
  state.events.modalOptions;
